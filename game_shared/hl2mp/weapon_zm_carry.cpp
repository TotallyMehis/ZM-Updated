...
/*#ifndef CLIENT_DLL
		...
		for ( int i = 0; i < children.Count(); ++i )
		{
			if ( children[i] )
			{
				DevMsg( "Enabling prop's %i child %i collision\n", pEntity->entindex(), children[i]->entindex() );
				
				// This perhaps could be a problem if the map already defines the prop as not solid.
				// Basically just enabling the collision here.
