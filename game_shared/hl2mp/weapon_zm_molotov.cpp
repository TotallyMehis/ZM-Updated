...
#define RETHROW_DELAY	0.5
	...
	switch( pEvent->event )
	{
		...
		case EVENT_WEAPON_THROW3:
			// FIXMOD_CHANGE - Mehis
			// Suppress useless messages. Make them dev messages instead.
			//Msg("Throwing molotov event received, throwfunction called!\n");
			DevMsg("Throwing molotov event received, throwfunction called!\n");
			

			// FIXMOD_CHANGE - Mehis
			// Certain animations on certain models crashes the client.
			// This only happens when the thrower is in the air.
			// This is a hacky way of fixing it.
		...
		if (pOwner)
		{
			...
			case GRENADE_PAUSED_SECONDARY:
				if( !(pOwner->m_nButtons & IN_ATTACK) && m_iExplosionStep >= 4) //LAWYER:  Make sure its ignited first
				{
					...
					DevMsg("ItemPostFrame sends throw anim!\n");
