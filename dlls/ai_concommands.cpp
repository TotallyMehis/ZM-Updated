...
// FIXMOD_CHANGE - Mehis
// Use global instead.
// This was causing crashes after the first round restart when this variable wasn't pointing anywhere. It is never actually made NULL like the code assumes.
...
{	
	...
	for(int i=0; i < gEntList.m_ZombieSelected.Count(); i++)
	{
		...
		if(pIterator && !newGroup->m_ZombieGroupMembers.HasElement(pIterator)) // You can never be too sure.
		{
			...
			// FIXMOD_CHANGE - Mehis
			// Now that we have our group, check if these zombies exist in other groups.
			// This will prevent group spamming.
			...
			for ( j = 0; j < len; j++ )
			{
				// Groups are stored as references.
				tempGroup = dynamic_cast<CZombieGroup*>( g_pZombieGroupManager->m_pZombieLists[j].Get() );
				...
				if ( tempGroup && (found = tempGroup->m_ZombieGroupMembers.Find( pIterator )) >= 0 )
				{
					...
					if ( !tempGroup->m_ZombieGroupMembers.Count() )
					{
						...
						g_pZombieGroupManager->SendGroupRemoveMsg( pPlayer, g_pZombieGroupManager->m_pZombieLists[j].GetSerialNumber() );

						// Remove the group from the list.
						g_pZombieGroupManager->m_pZombieLists.Remove( j );
						
						// Remove the group entity.
						UTIL_Remove( tempGroup );
	...
	g_pZombieGroupManager->AddGroup( newGroup, pPlayer);
...
void CC_ZM_GotoAndSelectSquad()
{
	...
	// FIXMOD_CHANGE - Mehis
	bool bViewSetup = false;

	for(int i=0; i < g_pZombieGroupManager->m_pZombieLists.Count(); i++)
	{
		int match = g_pZombieGroupManager->m_pZombieLists[i].GetSerialNumber();
		...
		if( match == serialNumber)
		{
			...
			CZombieGroup* zombieGroup = dynamic_cast<CZombieGroup*>(g_pZombieGroupManager->m_pZombieLists[i].Get());
	...
	// FIXMOD_CHANGE - Mehis
	// Only display if we selected something.
	// Also assign.
	if ( (pPlayer->m_iZombieSelected = gEntList.m_ZombieSelected.Count()) )
	{
