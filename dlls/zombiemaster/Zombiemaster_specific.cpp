...
Vector CZombieSpawn::FindValidSpawnPoint(void)
{
	...
	if (m_bDidSpawnSetup == false)
	{
		...
		if (pNodeEnt)
		{
			...
			// FIXMOD_CHANGE - Mehis
			// This fixes infinite recursion.
			// Faulty maps will link the last node with the first which causes it.
			int i;
			bool recursive = false;

			while (node != NULL)
			{
				for ( i = 0; i < nodePoints.Count(); i++ )
				{
					if ( nodePoints[i] == node )
					{
						Msg( "Detected recursion in spawn nodes!\n" );
