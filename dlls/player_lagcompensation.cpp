...
void CLagCompensationManager::StartLagCompensation( CBasePlayer *player, CUserCmd *cmd )
{
	...
	// FIXMOD_CHANGE - Mehis
	// Separate interp for NPCs...
#define DEFAULT_NPC_INTERP		0.25f

	int targettick_npc = cmd->tick_count - TIME_TO_TICKS( DEFAULT_NPC_INTERP );

	float correct_npc = clamp( correct + DEFAULT_NPC_INTERP, 0.0f, sv_maxunlag.GetFloat() );
	

	if ( fabs(correct_npc - TICKS_TO_TIME( gpGlobals->tickcount - targettick_npc )) > 0.2f )
	{
		targettick_npc = gpGlobals->tickcount - TIME_TO_TICKS( correct_npc );
	}
	...
	for ( int i = 0; i < nAIs; i++ )
	{
		...
		pNPC->m_bFlaggedForLagCompensation = false;
		...
		BacktrackEntity( pNPC, TICKS_TO_TIME( targettick_npc ) );
	}
}
		...
		if ( tr.startsolid || tr.allsolid )
		{
			...
			else
			{
				...
				if ( pHitEntity )
				{
					...
					//TGB: lagcomp in npc
					if (pNPC && !pNPC->m_bFlaggedForLagCompensation)
					{
						pNPC->m_bFlaggedForLagCompensation = true;
...
void CLagCompensationManager::BacktrackEntity( CAI_BaseNPC *pEntity, float flTargetTime )
{
	...
	//lagcomp in npc
	pEntity->m_bFlaggedForLagCompensation = true;
