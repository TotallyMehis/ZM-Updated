...
// FIXMOD_CHANGE - Mehis
// Keeps air time consistent.
#define AIRTIME						22 // What I tested, the player can stay underwater before taking damage for about 22 seconds.
...
void CBasePlayer::WaterMove()
{
	...
	// FIXMOD_CHANGE - Mehis
	// Keeps air time consistent.
	// Player's suit power would allow them to go for ~22 seconds.
	// If you drained all your suit power, next time you would only last for 7 seconds.
	// Some maps rely on this power to be more than just 7 seconds.
	// The flashlight does not rely on the suit power, however.
	...
	{	// fully under water
		...
		if (m_AirFinished < gpGlobals->curtime && !(GetFlags() & FL_GODMODE) )		// drown!
		{
			if (m_PainFinished < gpGlobals->curtime)
			{
				...
				// FIXMOD_CHANGE - Mehis
				// Since our air time is technically longer, here we increase the damage rate.
				//m_PainFinished = gpGlobals->curtime + 1;
				m_PainFinished = gpGlobals->curtime + 0.7f;
