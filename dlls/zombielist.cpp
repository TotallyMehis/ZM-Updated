...
// FIXMOD_CHANGE - Mehis
// Zombie group change.
CZombieGroupManager* g_pZombieGroupManager;
...
// FIXMOD_CHANGE - Mehis
void CZombieGroupManager::SendGroupRemoveMsg( CBasePlayer* pPlayer, int serial )
{
	if ( !pPlayer ) return;
	...
	UserMessageBegin( filter, "RemoveGroup" );
		WRITE_LONG( serial );
	MessageEnd();
}